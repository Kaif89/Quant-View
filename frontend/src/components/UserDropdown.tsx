...
export const UserDropdown: React.FC<UserDropdownProps> = ({ isOpen, onClose }) => {
  ...
  const userEmail = user?.primaryEmailAddress?.emailAddress || 'user@quantview.io';
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="p-4 flex items-center gap-4 bg-muted/10 border-t border-border mt-auto">
            {avatarUrl ? (
              <img src={avatarUrl} alt={userName} className="w-8 h-8 rounded-lg object-cover border border-border" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-xs font-bold tracking-tight truncate">{userName}</p>
                <div className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
              </div>
              <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest truncate">{userEmail}</p>
            </div>
          </div>
        </motion.div>
      )}
