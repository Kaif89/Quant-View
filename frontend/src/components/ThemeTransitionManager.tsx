...
import { useTheme, ThemeContext, resolveTheme } from '@/hooks/useTheme';
...
export function ThemeTransitionManager({ children }: { children: React.ReactNode }) {
  ...
  // Resolved theme for CSS class application (never 'system')
  const resolvedTheme = resolveTheme(isTransitioning ? (transitionTarget || theme) : theme);

  // Context isolation for children in the live layer
  // Pass the raw theme preference (can be 'system') so UI components can detect it
  const themeContextValue = {
    theme: isTransitioning ? (transitionTarget || theme) : theme,
    setTheme,
    toggleTheme,
    isTransitioning,
    transitionTarget,
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      */}
      <div 
        ref={liveAppRef}
        className={`${resolvedTheme} w-full h-full relative z-10 bg-background text-foreground`}
