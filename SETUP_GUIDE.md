...
> ```
> *(Or download the installer from Adoptium/Oracle)*
...
### Port already in use
```powershell
# Check what's using a port (e.g. port 5000)
Get-NetTCPConnection -LocalPort 5000

# Kill it by PID (replace <PID> with the OwningProcess ID from the command above)
...
```powershell
# Ctrl+C in each terminal, or use Stop-Process for the specific PIDs.
```
